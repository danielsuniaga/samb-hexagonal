...
import apis.services.checkenvolventml.ServicesCheckEnvolventML as ServicesCheckEnvolventML
import apis.services.deriv.ServicesDeriv as ServicesDeriv
import apis.services.methodologyenvolventml.ServicesMethodologyEnvolventML as ServicesMethodologyEnvolventML
...
class ControllerGetDataAnalysisDerivEnvolventML:
    ...
    ServicesApi = None
    ServicesSmtp = None
    ServicesCheckEnvolventML = None
    ServicesDeriv = None
    ServicesMethodologyEnvolventML = None
    ...
    def __init__(self):
        ...
        self.initialize_check_envolvent_services_interns()
    ...
    def initialize_services(self):
        ...
        self.ServicesCheckEnvolventML = ServicesCheckEnvolventML.ServicesCheckEnvolventML()
        self.ServicesDeriv = ServicesDeriv.ServicesDeriv()
        self.ServicesMethodologyEnvolventML = ServicesMethodologyEnvolventML.ServicesMethodologyEnvolventML()
    ...
    def initialize_check_envolvent_services_interns(self):
        ...
        self.ServicesCheckEnvolventML.init_services_deriv(self.ServicesDeriv)
        self.ServicesCheckEnvolventML.init_services_methodology_envolvent_ml(self.ServicesMethodologyEnvolventML)
        self.ServicesCheckEnvolventML.init_services_manager_days(self.ServicesManagerDays)
        self.ServicesCheckEnvolventML.init_services_indicators(self.ServicesIndicators)
        self.ServicesCheckEnvolventML.init_services_entrys_results(self.ServicesEntrysResults)
        self.ServicesCheckEnvolventML.init_services_movements(self.ServicesMovements)
        self.ServicesCheckEnvolventML.init_services_platform(self.ServicesPlatform)
        self.ServicesCheckEnvolventML.init_services_entrys(self.ServicesEntrys)
        self.ServicesCheckEnvolventML.init_services_indicators_entrys(self.ServicesIndicatorsEntrys)    
        self.ServicesCheckEnvolventML.init_services_cronjobs(self.ServicesCronjobs)
        self.ServicesCheckEnvolventML.init_services_telegram(self.ServicesTelegram)
        self.ServicesCheckEnvolventML.init_services_models(self.ServicesModels)
        self.ServicesCheckEnvolventML.init_services_methodologys(self.ServicesMethodologys)
        self.ServicesCheckEnvolventML.init_services_entrys_predict_models(self.ServicesEntrysPredictModels)

    def get_apis_name_envolvent_ml(self):
        return self.ServicesApi.get_apis_name_envolvent_ml()

    def set_apis_name_smtp(self):
        return self.ServicesSmtp.set_apis_name(self.get_apis_name_envolvent_ml())
    ...
    def init_verify_services(self, hour, date, id_cronjobs):
        return [
            ...
            },
            {
                ...
                'function': lambda: self.ServicesCronjobs.add_envolvent_ml(id_cronjobs, date),
                'send_email_on_error': True
            },
            {
                'name': 'models_service',
    ...
    async def initialize_deriv_services(self, date):
        ...
        result = await self.ServicesCheckEnvolventML.init()
        if not result['status']:
            return result
        self.ServicesEvents.set_events_field('init_broker', self.ServicesDates.get_current_date_mil_dynamic())
        await self.ServicesCheckEnvolventML.set_balance(self.ServicesDates.get_day())
        self.ServicesEvents.set_events_field('config_broker', self.ServicesDates.get_current_date_mil_dynamic())
        self.ServicesCheckEnvolventML.init_services_events(self.ServicesEvents)
        self.ServicesCheckEnvolventML.init_services_dates(self.ServicesDates)
        return {'status': True, 'message': 'Initialization successful'}
    
    async def process_deriv_services(self):
        await self.ServicesCheckEnvolventML.loops()
        await self.ServicesCheckEnvolventML.closed()
    ...
    async def GetDataAnalysisDerivEnvolventML(self, request):
