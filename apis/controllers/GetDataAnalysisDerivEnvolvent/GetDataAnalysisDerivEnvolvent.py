...
import apis.services.checkenvolvent.ServicesCheckEnvolvent as ServicesCheckEnvolvent
...
import apis.services.methodologyenvolvent.ServicesMethodologyEnvolvent as ServicesMethodologyEnvolvent

class ControllerGetDataAnalysisDerivEnvolvent:
    ...
    ServicesMethodologyEnvolvent = None
    ...
    def init_services(self):
        ...
        self.ServicesCheckEnvolvent = ServicesCheckEnvolvent.ServicesCheckEnvolvent()
        self.ServicesDeriv = ServicesDeriv.ServicesDeriv()
        self.ServicesMethodologyEnvolvent = ServicesMethodologyEnvolvent.ServicesMethodologyEnvolvent()
        self.ServicesManagerDays = ServicesManagerDays()
    ...
    def init_services_intern(self):
        ...
        self.ServicesCheckEnvolvent.init_services_deriv(self.ServicesDeriv)
        self.ServicesCheckEnvolvent.init_services_methodology_envolvent(self.ServicesMethodologyEnvolvent)
        self.ServicesCheckEnvolvent.init_services_manager_days(self.ServicesManagerDays)
        self.ServicesCheckEnvolvent.init_services_indicators(self.ServicesIndicators)
        self.ServicesCheckEnvolvent.init_services_entrys_results(self.ServicesEntrysResults)
        self.ServicesCheckEnvolvent.init_services_movements(self.ServicesMovements)
        self.ServicesCheckEnvolvent.init_services_platform(self.ServicesPlatform)
        self.ServicesCheckEnvolvent.init_services_entrys(self.ServicesEntrys)
        self.ServicesCheckEnvolvent.init_services_indicators_entrys(self.ServicesIndicatorsEntrys)
        self.ServicesCheckEnvolvent.init_services_cronjobs(self.ServicesCronjobs)
        self.ServicesCheckEnvolvent.init_services_telegram(self.ServicesTelegram)
        return True
    ...
    def get_apis_name_envolvent(self):
        return self.ServicesApi.get_apis_name_envolvent()

    def set_apis_name_smtp(self):
        return self.ServicesSmtp.set_apis_name(self.get_apis_name_envolvent())
    ...
    def verify_services(self, request, hour, date, id_cronjobs):
        servicios_a_verificar = [
            ...
            lambda: self.ServicesCronjobs.add_envolvent(id_cronjobs, date)
        ]
    ...
    async def initialize_deriv_services(self, date):
        ...
        result = await self.ServicesCheckEnvolvent.init()
        if not result['status']:
            return result
        self.ServicesEvents.set_events_field('init_broker', self.ServicesDates.get_current_date_mil_dynamic())
        await self.ServicesCheckEnvolvent.set_balance(self.ServicesDates.get_day())
        self.ServicesEvents.set_events_field('config_broker', self.ServicesDates.get_current_date_mil_dynamic())
        self.ServicesCheckEnvolvent.init_services_events(self.ServicesEvents)
        self.ServicesCheckEnvolvent.init_services_dates(self.ServicesDates)
        return {'status': True, 'message': 'Initialization successful'}

    async def process_deriv_services(self):
        await self.ServicesCheckEnvolvent.loops()
        await self.ServicesCheckEnvolvent.closed()
    ...
    async def GetDataAnalysisDerivEnvolvent(self, request):
        ...
        if not resultado_deriv['status']:
            self.ServicesSmtp.send_notification_email(date, resultado_deriv['message'])
