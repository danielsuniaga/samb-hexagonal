...
import apis.services.checkwmarecentml.ServicesCheckWMARecentML as ServicesCheckWMARecentML
import apis.services.deriv.ServicesDeriv as ServicesDeriv
import apis.services.methodologywmarecentml.ServicesMethodologyWMARecentML as ServicesMethodologyWMARecentML
...
class ControllerGetDataAnalysisDerivWMARecentML:
    ...
    ServicesApi = None
    ServicesSmtp = None
    ServicesCheckWMARecentML = None
    ServicesDeriv = None
    ServicesMethodologyWMARecentML = None
    ...
    def __init__(self):
        ...
        self.initialize_check_wma_recent_services_interns()
    ...
    def initialize_services(self):
        ...
        self.ServicesCheckWMARecentML = ServicesCheckWMARecentML.ServicesCheckWMARecentML()
        self.ServicesDeriv = ServicesDeriv.ServicesDeriv()
        self.ServicesMethodologyWMARecentML = ServicesMethodologyWMARecentML.ServicesMethodologyWMARecentML()
    ...
    def initialize_check_wma_recent_services_interns(self):
        ...
        self.ServicesCheckWMARecentML.init_services_deriv(self.ServicesDeriv)
        self.ServicesCheckWMARecentML.init_services_methodology_wma_recent_ml(self.ServicesMethodologyWMARecentML)
        self.ServicesCheckWMARecentML.init_services_manager_days(self.ServicesManagerDays)
        self.ServicesCheckWMARecentML.init_services_indicators(self.ServicesIndicators)
        self.ServicesCheckWMARecentML.init_services_entrys_results(self.ServicesEntrysResults)
        self.ServicesCheckWMARecentML.init_services_movements(self.ServicesMovements)
        self.ServicesCheckWMARecentML.init_services_platform(self.ServicesPlatform)
        self.ServicesCheckWMARecentML.init_services_entrys(self.ServicesEntrys)
        self.ServicesCheckWMARecentML.init_services_indicators_entrys(self.ServicesIndicatorsEntrys)    
        self.ServicesCheckWMARecentML.init_services_cronjobs(self.ServicesCronjobs)
        self.ServicesCheckWMARecentML.init_services_telegram(self.ServicesTelegram)
        self.ServicesCheckWMARecentML.init_services_models(self.ServicesModels)
        self.ServicesCheckWMARecentML.init_services_methodologys(self.ServicesMethodologys)
        self.ServicesCheckWMARecentML.init_services_entrys_predict_models(self.ServicesEntrysPredictModels)

    def get_apis_name_wma_recent_ml(self):
        return self.ServicesApi.get_apis_name_wma_recent_ml()

    def set_apis_name_smtp(self):
        return self.ServicesSmtp.set_apis_name(self.get_apis_name_wma_recent_ml())
    ...
    def init_verify_services(self, hour, date, id_cronjobs):
        return [
            ...
            },
            {
                ...
                'function': lambda: self.ServicesCronjobs.add_wma_recent_ml(id_cronjobs, date),
                'send_email_on_error': True
            },
            {
                'name': 'models_service',
    ...
    async def initialize_deriv_services(self, date):
        ...
        result = await self.ServicesCheckWMARecentML.init()
        if not result['status']:
            return result
        self.ServicesEvents.set_events_field('init_broker', self.ServicesDates.get_current_date_mil_dynamic())
        await self.ServicesCheckWMARecentML.set_balance(self.ServicesDates.get_day())
        self.ServicesEvents.set_events_field('config_broker', self.ServicesDates.get_current_date_mil_dynamic())
        self.ServicesCheckWMARecentML.init_services_events(self.ServicesEvents)
        self.ServicesCheckWMARecentML.init_services_dates(self.ServicesDates)
        return {'status': True, 'message': 'Initialization successful'}
    
    async def process_deriv_services(self):
        await self.ServicesCheckWMARecentML.loops()
        await self.ServicesCheckWMARecentML.closed()
    ...
    async def GetDataAnalysisDerivWMARecentML(self, request):
