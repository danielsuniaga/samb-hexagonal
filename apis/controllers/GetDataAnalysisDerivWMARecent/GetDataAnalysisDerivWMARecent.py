...
import apis.services.checkwmarecent.ServicesCheckWMARecent as ServicesCheckWMARecent
import apis.services.methodologywmarecent.ServicesMethodologyWMARecent as ServicesMethodologyWMARecent
...
class ControllerGetDataAnalysisDerivWMARecent:
    ...
    ServicesCheckWMARecent = None

    ServicesMethodologyWMARecent = None
    ...
    def initialize_services(self):
        ...
        self.ServicesCheckWMARecent = ServicesCheckWMARecent.ServicesCheckWMARecent() 

        self.ServicesMethodologyWMARecent = ServicesMethodologyWMARecent.ServicesMethodologyWMARecent()   
    ...
    def initialize_services_internal(self):
        ...
        self.ServicesCheckWMARecent.init_services_cronjobs(self.ServicesCronjobs)

        self.ServicesCheckWMARecent.init_services_deriv(self.ServicesDeriv)

        self.ServicesCheckWMARecent.init_services_methodology_wma_recent(self.ServicesMethodologyWMARecent)

        self.ServicesCheckWMARecent.init_services_manager_days(self.ServicesManagerDays)

        self.ServicesCheckWMARecent.init_services_indicators(self.ServicesIndicators)

        self.ServicesCheckWMARecent.init_services_entrys_results(self.ServicesEntrysResults)  

        self.ServicesCheckWMARecent.init_services_movements(self.ServicesMovements)

        self.ServicesCheckWMARecent.init_services_platform(self.ServicesPlatform)

        self.ServicesCheckWMARecent.init_services_dates(self.ServicesDates)

        self.ServicesCheckWMARecent.init_services_entrys(self.ServicesEntrys)

        self.ServicesCheckWMARecent.init_services_indicators_entrys(self.ServicesIndicatorsEntrys)

        self.ServicesCheckWMARecent.init_services_telegram(self.ServicesTelegram)

        return True
    
    def get_apis_name_wma_recent(self):

        return self.ServicesApi.get_apis_name_wma_recent()

    def set_apis_name_smtp(self):

        return self.ServicesSmtp.set_apis_name(self.get_apis_name_wma_recent())
    ...
    def verify_services(self, request, hour, date, id_cronjobs):
        ...
        servicios_a_verificar = [
            ...
            lambda: self.ServicesCronjobs.add_wma_recent(id_cronjobs, date)
        ]
    ...
    async def initialize_deriv_services(self, date):
        ...
        result = await self.ServicesCheckWMARecent.init()
        ...
        await self.ServicesCheckWMARecent.set_balance(self.ServicesDates.get_day())

        self.ServicesEvents.set_events_field('config_broker', self.ServicesDates.get_current_date_mil_dynamic())

        self.ServicesCheckWMARecent.init_services_events(self.ServicesEvents)
    ...
    async def process_deriv_services(self):
        ...
        await self.ServicesCheckWMARecent.loops()

        await self.ServicesCheckWMARecent.closed()
    ...
    async def GetDataAnalysisDerivWMARecent(self, request):
        ...
        if not resultado['status']:
            ...
            return self.ServicesSmtp.send_notification_email(date, resultado['message'])
