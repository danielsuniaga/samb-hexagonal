...
import apis.services.checkwmaml.ServicesCheckWMAML as ServicesCheckWMAML
import apis.services.managerdays.ServicesManagerDays as ServicesManagerDays
import apis.services.methodologywmaml.ServicesMethodologyWMAML as ServicesMethodologyWMAML
...
class ControllerGetDataAnalysisDerivWMAML:
    ServicesDates = None
    ServicesEvents = None
    ...
    ServicesCheckWMAML = None
    ServicesManagerDays = None
    ServicesMethodologyWMAML = None
    ...
    def initialize_services(self):
        ...
        self.ServicesCheckWMAML = ServicesCheckWMAML.ServicesCheckWMAML()
        self.ServicesManagerDays = ServicesManagerDays.ServicesManagerDays()
        self.ServicesMethodologyWMAML = ServicesMethodologyWMAML.ServicesMethodologyWMAML()
    ...
    def initialize_services_interns(self):
        ...
        self.ServicesCheckWMAML.init_services_manager_days(self.ServicesManagerDays)
        self.ServicesCheckWMAML.init_services_methodology_wma_ml(self.ServicesMethodologyWMAML)
        self.ServicesCheckWMAML.init_services_indicators(self.ServicesIndicators)
        self.ServicesCheckWMAML.init_services_entrys_results(self.ServicesEntrysResults)
        self.ServicesCheckWMAML.init_services_entrys(self.ServicesEntrys)
        self.ServicesCheckWMAML.init_services_cronjobs(self.ServicesCronjobs)
        self.ServicesCheckWMAML.init_services_platform(self.ServicesPlatform)
        self.ServicesCheckWMAML.init_services_indicators_entrys(self.ServicesIndicatorsEntrys)
        self.ServicesCheckWMAML.init_services_movements(self.ServicesMovements)
        self.ServicesCheckWMAML.init_services_telegram(self.ServicesTelegram)
        self.ServicesCheckWMAML.init_services_deriv(self.ServicesDeriv)
        self.ServicesCheckWMAML.init_services_models(self.ServicesModels)
        self.ServicesCheckWMAML.init_services_methodologys(self.ServicesMethodologys)
        self.ServicesCheckWMAML.init_services_entrys_predict_models(self.ServicesEntrysPredictModels)

    def get_apis_name_wma_ml(self):
        return self.ServicesApi.get_apis_name_wma_ml()

    def set_apis_name_smtp(self):
        return self.ServicesSmtp.set_apis_name(self.get_apis_name_wma_ml())
    ...
    def init_verify_services(self, hour, date, id_cronjobs):
        return [
            ...
            },
            {
                ...
                'function': lambda: self.ServicesCronjobs.add_wma_ml(id_cronjobs, date),
                'send_email_on_error': True
            },
            {
                'name': 'models_service',
    ...
    async def process_deriv_services(self):
        await self.ServicesCheckWMAML.loops()
        await self.ServicesCheckWMAML.closed()
    ...
    async def initialize_deriv_services(self, date):
        ...
        result = await self.ServicesCheckWMAML.init()
        if not result['status']:
            return result
        self.ServicesEvents.set_events_field('init_broker', self.ServicesDates.get_current_date_mil_dynamic())
        await self.ServicesCheckWMAML.set_balance(self.ServicesDates.get_day())
        self.ServicesEvents.set_events_field('config_broker', self.ServicesDates.get_current_date_mil_dynamic())
        self.ServicesCheckWMAML.init_services_events(self.ServicesEvents)
        self.ServicesCheckWMAML.init_services_dates(self.ServicesDates)
