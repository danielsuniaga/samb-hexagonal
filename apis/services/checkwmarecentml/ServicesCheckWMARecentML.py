...
class ServicesCheckWMARecentML:
    # --- Servicios base de CheckWMARecent ---
    ServicesDeriv = None
    ...
    # --- Servicios ML (heredados de TrendsExpansiveML) ---
    ServicesModels = None
    ServicesMethodologys = None
    ServicesEntrysPredictModels = None

    def get_project_name(self):
        return self.ServicesMethodologyWMARecentML.get_project_name()
    ...
    def init_services_methodology_wma_recent_ml(self, value):
        self.ServicesMethodologyWMARecentML = value
        return True
    ...
    def get_id_methodology(self):
        return self.ServicesMethodologyWMARecentML.get_id()
    ...
    def generate_candles(self, candles):
        return self.ServicesMethodologyWMARecentML.generate_candles(candles)
    ...
    def init_data_indicators(self, candles):
        return {
            ...
            'sma_short': self.get_sma_long(candles),  # Mantiene la lógica original de WMARecent
            'sma_long': self.get_sma_long(candles),
            'last_candle': self.get_candle_last(candles),
        }

    def set_indicators_methodology(self, indicators):
        return self.ServicesMethodologyWMARecentML.set_indicators(indicators)
    ...
    def check_candles(self, result):
        if not result:
            return False
        return self.ServicesMethodologyWMARecentML.check_candles()

    def get_indicators_methodology(self):
        return self.ServicesMethodologyWMARecentML.get_indicators()

    def check_rsi(self, rsi):
        return self.ServicesMethodologyWMARecentML.check_rsi(rsi)

    def check_sma(self, sma, last_candle):
        return self.ServicesMethodologyWMARecentML.check_sma(sma, last_candle)
    ...
    def check_result_indicators(self, result_indicators):
        return self.ServicesMethodologyWMARecentML.check_result_indicators(result_indicators)
    ...
    def check_monetary_filter_services(self, result):
        return self.ServicesMethodologyWMARecentML.check_monetary_filters(result)
    ...
    def get_type_entry(self):
        return self.ServicesMethodologyWMARecentML.get_type_entry_positions()
    ...
    def set_candles_positions(self, candles):
        return self.ServicesMethodologyWMARecentML.set_result_candles(candles)
    ...
    def get_candle_removed(self):
        return self.ServicesMethodologyWMARecentML.get_candle_removed()
    ...
    def get_condition_entry(self):
        return self.ServicesMethodologyWMARecentML.get_condition_entry()
    ...
    def set_result_positions_entity(self, result):
        return self.ServicesMethodologyWMARecentML.set_result_entrys(result)
    ...
    def get_result_entrys_result(self):
        return self.ServicesMethodologyWMARecentML.get_result_entrys_result()
    ...
    def get_name_methodology(self):
        return self.ServicesMethodologyWMARecentML.get_name()
    ...
    # --- Métodos ML (heredados de ServicesCheckTrendsExpansiveML) ---
    ...
    # --- Método principal loops (combinando WMARecent + ML) ---
    async def loops(self):
        ...
        logger.info(f"🎯 LOOPS EXECUTION | Execution: {execution_id} | Project: {project_name} | Method: loops | Methodology: {methodology} | Step: 6. Verificando modelo ML")
        result = self.check_predict_models(result, result_candles)
        logger.info(f"🎯 LOOPS EXECUTION | Execution: {execution_id} | Project: {project_name} | Method: loops | Methodology: {methodology} | Step: 6.1. Modelo ML | Valid: {bool(result)}")
